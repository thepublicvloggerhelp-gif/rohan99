            ...
            filtered.map(conv => {
              ...
              return (
                <Link key={conv.id} href={`/dm/${conv.id}`}
                  ...
                  {/* Avatar wrapper */}
                  <Avatar
                    url={conv.other?.avatar_url}
                    name={conv.other?.full_name ?? 'U'}
                    size={40}
                    wrapperClassName="relative flex-shrink-0"
                    containerClassName="w-10 h-10 rounded-full overflow-hidden bg-surface-4 flex items-center justify-center text-sm font-bold text-brand-400 border border-slate-200/60 shadow-sm animate-fade-in"
